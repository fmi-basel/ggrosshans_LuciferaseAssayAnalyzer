...
% Last Modified by GUIDE v2.5 20-Jan-2020 08:27:10
...
% UIWAIT makes gui_select_molts wait for user response (see UIRESUME)
...
if exist([filepath,filename,'.mat'],'file')
    ...
    elseif strcmpi(ext,'txt')
        ...
        %the Mikrowin2010 software. This might break if the format of the
...
if handles.current_sample < length(handles.samples)
    ...
    %SetHatchToZero(handles.SetHatchToZero,handles)
    try
...
guidata(hObject, handles);
save_data(handles)
% Hint: get(hObject,'Value') returns toggle state of ValidSample

function save_data(handles)
X=handles.X;
Xc=handles.Xc;
y=handles.y;
%label=handles.samples;
valid=handles.valid;
...
%save([handles.filepath,handles.filename,'.mat'],'X','Xc','y','valid','isControlSample','current_sample','samples','Molt','Hatch','sampleRate','annotated')
...
handles.Xc=Xc;
handles.y=y;
%handles.isControlSample=isControlSample;
%handles.samples=label;
handles.valid=valid;
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    %generates csv file with tab-delimited data of Hatch, MoltN_entry and MoltN_exit
...
tmp=unique(handles.samples);
    ...
    for i=1:length(tmp)
        ...
        %make heatmaps
    figure;imagesc(t,1:j-1,tmp_data_bin(ind(ind2),:));xlabel('Time (h)');ylabel('single worms');set(gca,'YTickLabel','');title(tmp{i});pause(0.1)
    figure;imagesc(t,1:j,log(tmp_data(ind(ind2),:)));xlabel('Time (h)');ylabel('single worms');set(gca,'YTickLabel','');title(tmp{i});colorbar;colormap(gray);pause(0.1)
        end
    end
    ...
    switch choice_include_hatch
        ...
        %make heatmaps
    figure;imagesc(t,1:j-1,tmp_data_bin(ind(ind2),:));xlabel('Time (h)');ylabel('single worms');set(gca,'YTickLabel','');pause(0.1)
    figure;imagesc(t,1:j,log(tmp_data(ind(ind2),:)));xlabel('Time (h)');ylabel('single worms');set(gca,'YTickLabel','');colorbar;colormap(gray);pause(0.1)
 end
...
if handles.valid(c)==1
    [handles.y(:,c) handles.Molt(:,:,c)]= detect_molts(handles.Xc(:,c),handles);
    %als er 4 molts zijn gevonden gewoon sorteren, anders de nullen op het
    %laatst zetten
...
%Boxplots according to defined order
    ...
    figure;boxplot(tmp_M(:),final_labels,'ColorGroup',repmat(handles.samples_ordered(ind),1,4),...
       'positions',box_positions,'LabelOrientation','inline');title('Molts');axis([0.5 box_positions(end)+0.5 0 1.25*max(tmp_M(:))])
    ...
    figure;boxplot(tmp_L(:),final_labels,'ColorGroup',repmat(handles.samples_ordered(ind),1,4),...
       'positions',box_positions,'LabelOrientation','inline');title('Larval stages');axis([0.5 box_positions(end)+0.5 0 1.25*max(tmp_L(:))]);
    ...
    figure;boxplot(tmp_I(:),final_labels,'ColorGroup',repmat(handles.samples_ordered(ind),1,4),...
       'positions',box_positions,'LabelOrientation','inline');title('Intermolts');axis([0.5 box_positions(end)+0.5 0 1.25*max(tmp_I(:))]);
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    %display statistics (t-test) of boxplots in the command line
    ...
    ind=y~=0 & handles.valid;
        ...
        %molts
        delta_time_median = (median(M(y==2 & handles.valid,:))-median(M(y==1 & handles.valid,:)))*handles.sampleRate;
        delta_time_mean = (mean(M(y==2 & handles.valid,:))-mean(M(y==1 & handles.valid,:)))*handles.sampleRate;
        perc_median =(delta_time_median(:)'./(median(M(y==1 & handles.valid,:))*handles.sampleRate))*100;
        perc_mean =(delta_time_mean(:)'./(mean(M(y==1 & handles.valid,:))*handles.sampleRate))*100;
        ...
        for i=1:4
            ...
            %two-sample t-test
            [h,p] = ttest2(M(y==1 & handles.valid,i),M(y==2 & handles.valid,i));
            [l,v] = vartest2(M(y==1 & handles.valid,i),M(y==2 & handles.valid,i)); %v: p-value of variance
            if p<0.001
        ...
        %Larval stages
        delta_time_median = (median(L(y==2 & handles.valid,:))-median(L(y==1 & handles.valid,:)))*handles.sampleRate;
        delta_time_mean = (mean(L(y==2 & handles.valid,:))-mean(L(y==1 & handles.valid,:)))*handles.sampleRate;
        perc_median =(delta_time_median(:)'./(median(L(y==1 & handles.valid,:))*handles.sampleRate))*100;
        perc_mean =(delta_time_mean(:)'./(mean(L(y==1 & handles.valid,:))*handles.sampleRate))*100;

        for i=1:4
           %Wilcoxon test
           %p = ranksum(L(y==1 & handles.valid,i),L(y==2 & handles.valid,i));

           %two-sample t-test
           [h,p] = ttest2(L(y==1 & handles.valid,i),L(y==2 & handles.valid,i));
           [l,v] = vartest2(L(y==1 & handles.valid,i),L(y==2 & handles.valid,i)); %v: p-value of variance
           if p<0.001
                disp(['Larval Stage ',num2str(i),': dT_median = ',num2str(delta_time_median(i),'%.2f'),' hrs (',num2str(perc_median(i), '%.1f'),'%), ','dT_mean = ',num2str(delta_time_mean(i),'%.2f'),' hrs (',num2str(perc_mean(i), '%.1f'),'%), ','t-test(p < 0.001)' ' ***', ', F-test (p = ',num2str(v,'%.3f'),')'])
            elseif p==0.001
                disp(['Larval Stage ',num2str(i),': dT_median = ',num2str(delta_time_median(i),'%.2f'),' hrs (',num2str(perc_median(i), '%.1f'),'%), ','dT_mean = ',num2str(delta_time_mean(i),'%.2f'),' hrs (',num2str(perc_mean(i), '%.1f'),'%), ','t-test(p = ',num2str(p,'%.3f'),')', ' ***', ', F-test (p = ',num2str(v,'%.3f'),')'])
            elseif (p<=0.01) && (p>0.001)
                disp(['Larval Stage ',num2str(i),': dT_median = ',num2str(delta_time_median(i),'%.2f'),' hrs (',num2str(perc_median(i), '%.1f'),'%), ','dT_mean = ',num2str(delta_time_mean(i),'%.2f'),' hrs (',num2str(perc_mean(i), '%.1f'),'%), ','t-test(p = ',num2str(p,'%.3f'),')', ' **', ', F-test (p = ',num2str(v,'%.3f'),')'])
            elseif p>0.05
                disp(['Larval Stage ',num2str(i),': dT_median = ',num2str(delta_time_median(i),'%.2f'),' hrs (',num2str(perc_median(i), '%.1f'),'%), ','dT_mean = ',num2str(delta_time_mean(i),'%.2f'),' hrs (',num2str(perc_mean(i), '%.1f'),'%), ','t-test(p = ',num2str(p,'%.3f'),')', ' ns', ', F-test (p = ',num2str(v,'%.3f'),')'])
            elseif (p<=0.05) && (p>0.01)
                disp(['Larval Stage ',num2str(i),': dT_median = ',num2str(delta_time_median(i),'%.2f'),' hrs (',num2str(perc_median(i), '%.1f'),'%), ','dT_mean = ',num2str(delta_time_mean(i),'%.2f'),' hrs (',num2str(perc_mean(i), '%.1f'),'%), ','t-test(p = ',num2str(p,'%.3f'),')', ' *', ', F-test (p = ',num2str(v,'%.3f'),')'])
            end
        end
        ...
        %Intermolt
        delta_time_median = (median(I(y==2 & handles.valid,:))-median(I(y==1 & handles.valid,:)))*handles.sampleRate;
        delta_time_mean = (mean(I(y==2 & handles.valid,:))-mean(I(y==1 & handles.valid,:)))*handles.sampleRate;
        perc_median =(delta_time_median(:)'./(median(I(y==1 & handles.valid,:))*handles.sampleRate))*100;
        perc_mean =(delta_time_mean(:)'./(mean(I(y==1 & handles.valid,:))*handles.sampleRate))*100;

        for i=1:4
            %Wilcoxon test
            %p = ranksum(I(y==1 & handles.valid,i),I(y==2 & handles.valid,i));

            %two-sample t-test
            [h,p] = ttest2(I(y==1 & handles.valid,i),I(y==2 & handles.valid,i));
            [l,v] = vartest2(I(y==1 & handles.valid,i),I(y==2 & handles.valid,i)); %v: p-value of variance
            if p<0.001
                disp(['Intermolt ',num2str(i),': dT_median = ',num2str(delta_time_median(i),'%.2f'),' hrs (',num2str(perc_median(i), '%.1f'),'%), ','dT_mean = ',num2str(delta_time_mean(i),'%.2f'),' hrs (',num2str(perc_mean(i), '%.1f'),'%), ','t-test(p < 0.001)' ' ***', ', F-test (p = ',num2str(v,'%.3f'),')'])
            elseif p==0.001
                disp(['Intermolt ',num2str(i),': dT_median = ',num2str(delta_time_median(i),'%.2f'),' hrs (',num2str(perc_median(i), '%.1f'),'%), ','dT_mean = ',num2str(delta_time_mean(i),'%.2f'),' hrs (',num2str(perc_mean(i), '%.1f'),'%), ','t-test(p = ',num2str(p,'%.3f'),')', ' ***', ', F-test (p = ',num2str(v,'%.3f'),')'])
            elseif (p<=0.01) && (p>0.001)
                disp(['Intermolt ',num2str(i),': dT_median = ',num2str(delta_time_median(i),'%.2f'),' hrs (',num2str(perc_median(i), '%.1f'),'%), ','dT_mean = ',num2str(delta_time_mean(i),'%.2f'),' hrs (',num2str(perc_mean(i), '%.1f'),'%), ','t-test(p = ',num2str(p,'%.3f'),')', ' **', ', F-test (p = ',num2str(v,'%.3f'),')'])
            elseif p>0.05
                disp(['Intermolt ',num2str(i),': dT_median = ',num2str(delta_time_median(i),'%.2f'),' hrs (',num2str(perc_median(i), '%.1f'),'%), ','dT_mean = ',num2str(delta_time_mean(i),'%.2f'),' hrs (',num2str(perc_mean(i), '%.1f'),'%), ','t-test(p = ',num2str(p,'%.3f'),')', ' ns', ', F-test (p = ',num2str(v,'%.3f'),')'])
            elseif (p<=0.05) && (p>0.01)
                disp(['Intermolt ',num2str(i),': dT_median = ',num2str(delta_time_median(i),'%.2f'),' hrs (',num2str(perc_median(i), '%.1f'),'%), ','dT_mean = ',num2str(delta_time_mean(i),'%.2f'),' hrs (',num2str(perc_mean(i), '%.1f'),'%), ','t-test(p = ',num2str(p,'%.3f'),')', ' *', ', F-test (p = ',num2str(v,'%.3f'),')'])
            end
    end
...
if handles.SetHatchToZero.Value==1 %if box is checked, the hatch of all samples is set to zero
    handles.Hatch(:)=0;
else %if box is unchecked, annotate the hatch
        for i=1:size(handles.X,2)
        try;handles.Hatch(i)=find(1:length(handles.X(:,i))>3 & handles.X(:,i)'>(mean(handles.X(1:20,i))+5*std(handles.X(1:20,i))),1,'first')-1;end
        end
end
...
Formats(1).type = 'list';
Formats(1).style = 'listbox';
Formats(1).items = handles.uniq_samples;
Formats(1).format = 'text';
Formats(1).limits = [0 length(Formats(1).items)];
Formats(1).size = [100 length(Formats(1).items)*15];
[cfg,Cancelled] = inputsdlg(Prompt,'Select Strains',Formats);
...
%Boxplots
   ind = logical(zeros(size(handles.samples)));
for i=1:length(cfg{1})
   ind(strcmp(handles.samples,handles.uniq_samples(cfg{1}(i))) & handles.valid)=1;
end
   labels=repmat({'',' ','  ','   '},sum(ind),1);labels=labels(:)';
   final_labels = strcat(repmat(handles.samples(ind),1,4),labels);
   clear labels
    if is_even
            tick_positions = box_positions(ceil(length(cfg{1})/2)+[0:3]*length(cfg{1}))+0.5;
        else
   ...
   tmp_Lumi = lumi(ind,:);
   figure;boxplot(tmp_Lumi(:),final_labels,'ColorGroup',repmat(handles.samples(ind),1,4),...
       'positions',box_positions,'LabelOrientation','inline');title('Median Luminescence during Molt');axis([0.5 box_positions(end)+0.5 0 1.25*max(tmp_Lumi(:))])
